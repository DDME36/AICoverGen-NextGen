...
def run_mdx(model_params, output_dir, model_path, filename, exclude_main=False, exclude_inversion=False, suffix=None, invert_suffix=None, denoise=False, keep_orig=True, m_threads=2):
    ...
    # Fallback parameters if model hash not found
    if mp is None:
        model_name = os.path.basename(model_path)
        print(f"⚠️  Warning: Model hash {model_hash} not found for {model_name}")
        print(f"   Using fallback parameters for {model_name}...")
        
        # Specific fallbacks for known models
        if "KARA" in model_name:
            # UVR_MDXNET_KARA_2.onnx - Backup vocals separator
            mp = {
                "compensate": 1.035,
                "mdx_dim_f_set": 2048,
        ...
        elif "Reverb" in model_name or "reverb" in model_name:
        ...
        elif "Voc" in model_name or "vocal" in model_name.lower():
